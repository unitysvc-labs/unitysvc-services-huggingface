...
For every ``offering.json`` under ``data/huggingface/services/*/`` with
...
SERVICES_DIR = REPO_ROOT / "data" / "huggingface" / "services"
...
def main() -> None:
    ...
    try:
        ...
        for path in offerings:
            ...
            raw_model_id = offering.get("name") or path.parent.name.replace(
                "-byok", ""
            ).replace("-byop", "")
            model_id = _hf_canonical_id(raw_model_id)
