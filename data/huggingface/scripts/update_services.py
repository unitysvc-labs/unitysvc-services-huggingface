#!/usr/bin/env python3
"""
Simplified update_services.py for Hugging Face using unitysvc_services data builders.
...
class ModelExtractor:
    ...
    def determine_service_type(self, model_id: str) -> str:
        ...
        if any(kw in model_lower for kw in ["embed", "embedding", "sentence-transformer"]):
            return "embedding"
        if any(kw in model_lower for kw in ["rerank"]):
            return "rerank"
        if any(kw in model_lower for kw in ["vision", "image-to-text"]):
            return "vision_language_model"
        if any(kw in model_lower for kw in ["whisper", "speech-to-text"]):
            return "prerecorded_transcription"
        if any(kw in model_lower for kw in ["tts", "text-to-speech"]):
            return "text_to_speech"
        if any(kw in model_lower for kw in ["stable-diffusion", "flux", "image-generation"]):
    ...
    def get_code_examples(self, model_id: str) -> list[tuple[str, str, str]]:
        ...
        if any(kw in model_lower for kw in ["sentence-transformer", "embed"]):
            return [
                ("Python code example", "../../docs/code-example-sentencetransformers.py.j2", "python"),
                ("JavaScript code example", "../../docs/code-example-sentencetransformers.js.j2", "javascript"),
                ("cURL code example", "../../docs/code-example-sentencetransformers.sh.j2", "bash"),
            ]
        if any(kw in model_lower for kw in ["stable-diffusion", "flux"]):
            return [
    ...
    def build_listing(self, model_id: str) -> ListingDataBuilder:
        ...
        builder.set_raw("service_options", {"default_parameters": {"api_key": "${ secrets.HF_TOKEN }"}})
